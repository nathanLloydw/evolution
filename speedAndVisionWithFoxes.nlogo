...
rabbits-own [energy-current energy-max energy-per-berry age age-max vision speed trait-points reproduced gen]
...
to go
  ...
  if ticks > 0 and ticks mod 800 = 0
  [
...
to move-rabbits
  ...
  [
    ...
    ifelse any? foxes in-radius vision
    [
    ...
    [
      ...
      if any? burrows-here
      [
       hatch-rabbits ((random 4) + 1)
       [

          let tmp (random 2)
          let point ((random 2) + 1)

          ifelse vision + speed + point < trait-points
          [
            if tmp = 0
            [
          ...
          [
            if tmp = 0 and (speed - point) > 0
            [
              set vision (vision + point)
              set speed (speed - point )

            ]
            if tmp = 1 and (vision - point) > 0
            [
              set speed (speed + point)
              set vision (vision - point)
         ...
         set energy-current 100
         set reproduced false
         set color rgb (260 - (vision * 8)) (260 - (speed * 8)) 255
         set gen (gen + 1)
       ]
      set reproduced true
      ]
...
"vision" 1.0 0 -13840069 true "" "plot sum [vision] of rabbits / count rabbits"
